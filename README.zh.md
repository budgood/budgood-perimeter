...
**缘起——它不是为了优化"Loop Engineering"或任何 agent 框架而做的。** 它是作者为治理一个长期演化的学术知识库而搭的真实系统里抽出来的;当时反复咬人的是一个很实在的问题:*现在到底有多少东西在读这个知识库,而每条记录的等级——它"未核实"/"低置信"/"candidate"的身份——在经过这些读取者时还活着吗?* 这工具存在,是因为这个问题真实、反复、且没人负责,不是因为 agent 圈的什么风潮。
