...
**缘起——它不是为了优化"Loop Engineering"或任何 agent 框架而做的。** 它是作者从治理自己精心打造的**寻思者研究型知识图谱**的实践里抽出来的——这个库受唯识学阿赖耶识"种现生熏"思想启发而开发(藏识中,种子生现行、现行熏种子,辗转相依),内置一套独特的反幻觉机制。其设计也把系统自身的「身份」(此刻哪个模式在主事)如此对待——即「身份 = 被增上缘辅助、但终究内生的拣择现行」:它不是静态 persona,也不是把拣择外包给查表,而是受证据、规则、经验种子(增上缘)辅助、却终究由模型当下现行内生完成的抉择;一张 append-only 的「所缘特征 → 该警觉什么」触发表只把当下风险**照亮**,从不替模型路由、决定或执行。当时反复咬人的有两个很实在的问题。其一:*现在到底有多少东西在读这个知识库?*——时间一长,脚本、服务、agent 越积越多,这份"读取者清单"早就超出谁的脑子能记的范围。其二,更隐蔽:每条记录都带着一个**等级**,就是"未核实""低置信""candidate"这样的标记,告诉你*这条该信几分*。当一个读取者把这条记录取出来、往下游传时,它**是把这个标记一并带下去,还是悄悄把它丢了?**——一旦丢了,一个本该被打问号的猜测,就摇身变成看起来确定的事实。这工具存在,是因为这两个问题真实、反复、且没人负责,不是因为 agent 圈的什么风潮。

budgood-perimeter 是作者称为 **缘起工程 / Condition Engineering** 的一套小设计学派里的第一个工具——这套学派本身扎根于古典范畴(亲因 vs 增上缘),而非 agent 框架。一个念头贯穿全部:
...
**关于 agent 循环——一个值得细画的对照(虽非动机)。** 它不是为修 agent 的"循环工程"而做的,但把两者并排看确实很有启发,因为它们在"判断该活在哪里"上下了**相反的注**:循环工程把判断**推出**模型、塞进脚手架——eval、router、verifier 自动决策并收敛;而这套工具把判断**留在**现行,数据只用来让风险无法被忽略。

有价值的是两种下注**失效形态相反**。硬化的脚手架失效时**静默且系统性**:世界一变,过时的判别器照旧决策、自信地错,还把这次的错**制度化**成规则——那个真实事故就是写照:上游改了输出格式,校验器普遍失败,约 90% 流量被悄悄升级到最贵模型,不报错不告警。把判断留在现行则失效**局部且随机**:某次看走眼,但不被冻成规则。所以闭环只是**一个旋钮的某一档**——oracle 又硬又便宜、动作可逆时它对,否则它是渎职。budgood-perimeter 就是这旋钮另一端的实物。完整校准理论、以及这条"提示词工程 → 上下文工程 → 受治边界"的谱系,见 [`docs/framework.md`](docs/framework.md)。
...
有问题、想法,或想交流知识库治理的经验——尤其欢迎佛学知识库、代码治理方向的交流?欢迎联系我(**释则生**):
