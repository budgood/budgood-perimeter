...
**缘起——它不是为了优化"Loop Engineering"或任何 agent 框架而做的。** 它是作者为治理一个长期演化的学术知识库而搭的真实系统里抽出来的。当时反复咬人的有两个很实在的问题。其一:*现在到底有多少东西在读这个知识库?*——时间一长,脚本、服务、agent 越积越多,这份"读取者清单"早就超出谁的脑子能记的范围。其二,更隐蔽:每条记录都带着一个**等级**,就是"未核实""低置信""candidate"这样的标记,告诉你*这条该信几分*。当一个读取者把这条记录取出来、往下游传时,它**是把这个标记一并带下去,还是悄悄把它丢了?**——一旦丢了,一个本该被打问号的猜测,就摇身变成看起来确定的事实。这工具存在,是因为这两个问题真实、反复、且没人负责,不是因为 agent 圈的什么风潮。
